...
@section('main-content')
    <div class="row">
        ...
        <div class="col-md-5">
            <div class="info-box bg-{{ fdate(App\Member::find($member)->ingress, 'd', 'Y-m-d') > date('d') ? 'green': 'red'}}">
                <span class="info-box-icon"><i class="fa fa-credit-card"></i></span>

                <div class="info-box-content">
                    <span class="info-box-text">Corte</span>
                    <span class="info-box-number">
                        {{ fdate(App\Member::find($member)->ingress, 'd', 'Y-m-d') }}
                        {{ fdate(date('Y-m-d'), '\d\e F', 'Y-m-d') }}
                    </span>
                    <span class="progress-description">
                        {{ fdate(App\Member::find($member)->ingress, 'd', 'Y-m-d') > date('d') ? 'A TIEMPO': 'ATRASADO'}}
                    </span>
                </div>
            </div>
